...
class AmazonOrderList extends AmazonOrderCore implements Iterator
{
    ...
    public function resetMarketplaceFilter()
    {
        ...
        if (array_key_exists('marketplaceId', $this->s)) {
            $this->options['MarketplaceId.Id.1'] = $this->s['marketplaceId'];
        } else {
    ...
    protected function parseXML($xml)
    {
        ...
        foreach ($xml->Orders->children() as $key => $data) {
            ...
            $this->orderList[$this->index] = new AmazonOrder($this->s, null, $data, $this->mockMode, $this->mockFiles, $this->config);

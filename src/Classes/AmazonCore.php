...
abstract class AmazonCore
{
    ...
    {
        if (is_null($config)) {
            $config = config('amazon-mws.config');
        }
...
<ErrorResponse xmlns="http://mws.amazonaws.com/doc/2009-01-01/">
  ...
  <RequestID>123</RequestID>
     ...
     * @param array $config <p>The config array.</p>
     ...
     * @param array $s [optional] <p>The store name to look for.
    ...
    public function setStore(array $s = null)
    {
        if (!is_null($s)) {
            ...
            if (array_key_exists('keyId', $s)) {
                $this->options['AWSAccessKeyId'] = $s['keyId'];
            } else {
                $this->log("Access Key ID is missing!", 'Warning');
            }
            if (!array_key_exists('secretKey', $s)) {
                $this->log("Secret Key is missing!", 'Warning');
            }
            if (!empty($s['serviceUrl'])) {
                $this->urlbase = $s['serviceUrl'];
            }
            if (!empty($s['MWSAuthToken'])) {
                $this->options['MWSAuthToken'] = $s['MWSAuthToken'];
            }
        } else {
